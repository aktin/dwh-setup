...
echo
echo +++++ STEP 2 +++++ HTTP Reverse Proxy Konfigurierung | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 2.1 +++++ HTTP Webroot Finden | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 3.01 +++++ Log Ordner | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 3.02 +++++ Wildfly Anpassung | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 4 +++++ Installation via ANT | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 5 +++++ Set Up Wildfly in autostart and start it | tee -a $LOGFILE
echo
##### Set up wildfly
# Create user
...
echo
echo +++++ STEP 6.01 +++++  Create /var/lib/aktin   | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 6.02 +++++  Aktin.properties nach Wildfly kopieren | tee -a $LOGFILE
echo
cp -v $INSTALL_ROOT/lib/aktin.properties $WILDFLY_HOME/standalone/configuration/  2>&1 | tee -a $LOGFILE


echo
echo +++++ STEP 6.03 +++++ Update local DWH ontology | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 6.04 +++++ Create AKTIN Database in postgres  | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 6.05 +++++ Create Aktin Data source in wildfly  | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 7 +++++  Email Einrichtung | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 9 +++++  restart Wildfly Service | tee -a $LOGFILE
echo
...
echo
echo +++++ STEP 9 +++++  Deploy new dwh-j2ee EAR | tee -a $LOGFILE
echo
